		...
		for (int i = 0; i < empresas.size(); i++) {
			...
			Empresa empresa = empresas.get(i);
				...
				if(existe) {
				 ...
				 newEmpresa=new EmpresaServices().createEmpresa(empresa.getNombre());
				...
				Cuenta cuenta = cuentas.get(i);
					...
					for (int j = 0; j < cuentasC.size(); j++) {
						...
						Cuenta cuentaC = cuentasC.get(j);
							...
							if((cuenta.getNombre().equals(cuentaC.getNombre()) & cuenta.getPeriodo().equals(cuentaC.getPeriodo()))) {
